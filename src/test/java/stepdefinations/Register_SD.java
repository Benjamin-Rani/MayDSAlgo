...
public class Register_SD {
	...
	throws InterruptedException {
	 ...
	 /* @When("^The user enter sheet \"([^\"]*)\" and (.+)$")
	    public void the_user_enter_sheet_something_and(String sheetname, int rownumber, String strArg1) 
	    throws InvalidFormatException, IOException {
	    
		 ExcelReader reader = new ExcelReader();

		List<Map<String, String>> testdata = reader.getData(Excelpath, sheetname);
		excelusername = testdata.get(rownumber).get("excelusername");
		excelpassword = testdata.get(rownumber).get("excelpassword");
		
		System.out.println("New user name n password --- " + excelusername + excelpassword);
